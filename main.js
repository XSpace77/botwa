...
// Pesan koneksi
console.log(`[${config.pkg.name}] Menyambung Ai Detector...`);
...
bot.ev.once(Events.ClientReady, async (m) => {
    console.log(`[${config.pkg.name}] Ai Detector Siap ${m.user.id}`);
...
bot.ev.on(Events.MessagesUpsert, async (m, ctx) => {
    ...
    // Log pesan masuk
    if (isGroup) {
        console.log(`[${config.pkg.name}] Ada Pesan Dari: ${groupId}, by: ${senderId}`);
    } else {
        console.log(`[${config.pkg.name}] Ada Pesan Dari: ${senderId}`);
    }
